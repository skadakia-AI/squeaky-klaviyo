...
type LoadJDResult =
  | { success: true; rawText: string; sparse: boolean }
  | { success: false; error: string; message: string }

function looksLikeJD(text: string): boolean {
  const words = text.trim().split(/\s+/).length
  if (words < 80) return false
  const signals = ['responsibilities', 'requirements', 'qualifications', 'experience', 'role', 'position', 'job', 'skills']
...
export async function loadJD(input: LoadJDInput): Promise<LoadJDResult> {
  ...
  // ── 2. Validate ──────────────────────────────────────────────────────────
  const sparse = !looksLikeJD(rawText)
  ...
  return { success: true, rawText, sparse }
}
